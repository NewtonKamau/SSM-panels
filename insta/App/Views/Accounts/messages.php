...
$(document).ready( function() {
  ...
  $('#accountResults').on('click', '.load-messages', function() {
    
    var typeMessage = $('#postMessages').val();
    if(jQuery.inArray(typeMessage, messages) == -1) {
      messages.push(typeMessage);
    }
    $('.tags-elements').html('<ul class="tags-lists">'+add_messages_elements()+'</ul>');
    $('#postMessages').val('');
    content_rescroll();
  ...
  $('#accountResults').on('click', '.close-tag', function() {
    ...
    messages = jQuery.grep(messages, function(value) {
  ...
  $('#accountResults').on('click', '.schedule', function() {
    ...
    $.post('{{ site_path }}/task/addtask', {"lists":messages,"action":"automessage","accid":activeId,"delay":"1m"}, function(data) {
  ...
  $('.aside-list-item').on('click', function() {
    ...
    $.post('{{ site_path }}/accounts/loadaccount/messages', {"accid":accId}, function(data) {
  ...
  function add_messages_elements() {
    var html = '';
  ...
  <nav class="sidebar-scroll" id="sidebar">
    <div class="hor-menu">
        ...
        <ul class="mt-3 mb-5">
          ...
          <li data-type="accounts" data-sidebar="direct" data-icons="messages" title="Direct Message" class="sidebar-link title-tooltip active"></li>
          <hr>
...
<div class="content-scroll" id="content" style="width:100%;">
  <nav id="topNav" class="navbar navbar-expand-md navbar-light">
      <h1 class="topbar-title">Direct Message all followers/following</h1>
