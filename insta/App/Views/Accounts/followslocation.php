...
$(document).ready( function() {
  ...
  $('#accountResults').on('click', '.load-button', function() {
    ...
    var typeLocation = $('#postLocations').val();
    
    $.post('{{ site_path }}/task/loadplaces', {"accid":activeId, "keyword":typeLocation}, function(data) {
      var array = jQuery.parseJSON(data);
      $('.locations-elements').empty();
      locations = [];
      locationsIds = [];    
      
      if(array.stat == 'success') {
        jQuery.each(array.elements,function(key,val){
          if(jQuery.inArray(key, locationsIds) == -1) {
            locations.push(val);
            locationsIds.push(key);
          }
        });
        $('.locations-elements').html('<ul class="tags-lists">'+add_locations_elements()+'</ul>');
      } else {
  ...
  $('#accountResults').on('click', '.close-location', function() {
    var closeLocation = $(this).attr('data-location');
    var closeName = $(this).attr('data-text');

    locationsIds = jQuery.grep(locationsIds, function(value) {
      return value != closeLocation;
    });
    locations = jQuery.grep(locations, function(value) {
      return value != closeName;
    });
  ...
  $('#accountResults').on('click', '.schedule', function() {
    ...
    $.post('{{ site_path }}/task/addtask', {"lists":locationsIds,"action":"autofollowlocations","accid":activeId,"delay":"1h","threads":maxHour,"max":maxDay,"job_max":maxTotal}, function(data) {
  ...
  $('.aside-list-item').on('click', function() {
    ...
    $.post('{{ site_path }}/accounts/loadaccount/followslocation', {"accid":accId}, function(data) {
  ...
  function add_locations_elements() {
    var html = '';
    var i = 0;
    $.each(locations, function( index, value ) {
      if(i == 3) {
        html += '<li>'+value+'<img src="{{ site_path }}/img/icons/close.png" class="ml-2 close-location" data-location="'+locationsIds[index]+'" data-text="'+value+'"/></li><br>';
        i = 0;
      } else {
        html += '<li>'+value+'<img src="{{ site_path }}/img/icons/close.png" class="ml-2 close-location" data-location="'+locationsIds[index]+'" data-text="'+value+'"/></li>';
      }
      i++;
    });
    return html;
  ...
  <nav class="sidebar-scroll" id="sidebar">
    <div class="hor-menu">
        ...
        <ul class="mt-3 mb-5">
          ...
          <li data-type="accounts" data-sidebar="follows" data-icons="follows" title="Auto Follows" data-tooltip-content="#tooltip_content" class="sidebar-link content-tooltip active"></li>
      ...
      <div id="tooltip_content">
        <div class="sidebar-sub-links">
          <ul>
            ...
            <li class="active"><a href="{{ site_path }}/accounts/follows/location">Autofollow by Location</a></li>
...
<div class="content-scroll" id="content" style="width:100%;">
  <nav id="topNav" class="navbar navbar-expand-md navbar-light">
      <h1 class="topbar-title">Get Follows</h1>
